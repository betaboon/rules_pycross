...
PycrossBuildExecRuntimeInfo = provider(
    ...
    fields = {
        ...
        "exec_sys_path": "An array of system path directories (i.e., the value of sys.path from `python -m site`).",
...
def find_hermetic_sys_path(py_info):
    interpreter_path = py_info.interpreter.path
    base = paths.dirname(paths.dirname(interpreter_path))
    return [paths.join(base, "lib/python3.9/")]
...
def _pycross_hermetic_toolchain_impl(ctx):
    ...
    pycross_info = PycrossBuildExecRuntimeInfo(
        ...
        exec_sys_path=find_hermetic_sys_path(exec_py_info),
